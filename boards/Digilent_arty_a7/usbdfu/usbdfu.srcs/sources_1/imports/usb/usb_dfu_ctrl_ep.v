...
genvar alt_num_alt;
generate
  for (alt_num_alt = 0; alt_num_alt < ALT_MODE_MAX; alt_num_alt = alt_num_alt + 1) begin
    initial begin
      cfg_rom[alt_num_alt*9 + 'h09] <= 9;     // bLength
      cfg_rom[alt_num_alt*9 + 'h0A] <= 4;     // bDescriptorType
      cfg_rom[alt_num_alt*9 + 'h0B] <= 0;     // bInterfaceNumber
      cfg_rom[alt_num_alt*9 + 'h0C] <= alt_num_alt; // bAlternateSetting
      cfg_rom[alt_num_alt*9 + 'h0D] <= 0;     // bNumEndpoints
      cfg_rom[alt_num_alt*9 + 'h0E] <= 'hFE;  // bInterfaceClass (Application Specific Class Code)
      cfg_rom[alt_num_alt*9 + 'h0F] <= 1;     // bInterfaceSubClass (Device Firmware Upgrade Code)
      cfg_rom[alt_num_alt*9 + 'h10] <= 2;     // bInterfaceProtocol (DFU mode protocol)
      cfg_rom[alt_num_alt*9 + 'h11] <= (STR_INDEX_PARTITIONS + alt_num_alt); // iInterface
    end
  end
endgenerate
